...
- **Duplicate Account Detection**: Pre-Flight fuzzy name matching detects near-duplicate account names (substring, prefix, first+last token matching) across contracts. Surfaces `DUPLICATE_ACCOUNT_NAME` blocker items in Pre-Flight with a resolution modal: "Same Person — Merge" updates all workbook account name fields to the chosen canonical name and re-condenses P1D.1 rows; "Different People — Dismiss" suppresses the pair for the session. Dismissed pairs tracked in `_dismissedDuplicatePairs`. Audit events emitted on resolution.